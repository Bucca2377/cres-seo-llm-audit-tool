...
- REPORT THE LOWEST AVAILABLE RENT. When the availability panel lists INDIVIDUAL available units with their own rents (e.g. a floor plan headlined "$1,049" but with unit #708 at $999 among the available units), report the LOWEST currently-available rent for that floor plan — the cheapest unit a prospect can actually lease right now — NOT just the "starting at" / headline number. Phrase it as "from $999" or a range with the count, e.g. "2 Bed from $999 ($999-$1,049, 6 available)". If only a single "starting at" / headline price is shown (no per-unit rents), report that. Keep the available-unit COUNT when shown.
      ...
      if (gbpHadData && (!officeHours || Object.keys(officeHours).length === 0)) {
        ...
        if (hoursRow && hoursRow.google?.status !== "green") {
          const haveElsewhere =
            ...
            !!(aptRead?.ok && Object.keys(aptRead.officeHours || {}).length > 0);
          hoursRow.google = {
            status: "red",
            note: haveElsewhere
              ? "No office hours on the Google Business Profile. Add them so searchers see when the office is open — your website and Apartments.com already list the hours, so copy those exact times."
